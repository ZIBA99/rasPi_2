...
#include <sys/ioctl.h>
#include <signal.h>  // signal 처리를 위한 헤더 추가
#include <stdbool.h> // bool 타입을 위한 헤더 추가
...
int main( )
{
    ...
    printf("센서 데이터 읽기 시작... (종료하려면 Ctrl+C를 누르세요)\n");

    // running 플래그가 true인 동안 계속 실행
    while(running) {
        int ax, ay, az;
        int gx, gy, gz;
        int mx, my, mz;
